...
#     The extracted text is chunked and stored alongside the ingested logs.
#     Multiple files are accepted in a single call.
#     """
...
#                 import io
#                 df_kb = pd.read_csv(io.BytesIO(content))
...
#                 errors.append(f"'{upload.filename}': could not extract text.")
#                 continue
...
#         except Exception as exc:
#             errors.append(f"'{upload.filename}': {str(exc)}")

#     # Persist KB chunks to MongoDB alongside existing SDCC doc
#     sdcc_collection.update_one(
#         {"ai_name": ai_name, "owner_id": str(current_user["_id"])},
#         {"$set": {
...
#         "errors":          errors,
#         "message": (
#             f"Knowledge base updated: {len(all_chunks)} chunks from "
...
#     "avg_coherence":      "sentence-length + TTR + discourse marker heuristic",
#     "avg_perplexity":     "vocabulary entropy proxy",
#     "roc_auc":            "LLM-judge labels → scikit-learn roc_auc_score",
#     "f1_score":           "LLM-judge labels → scikit-learn f1_score",
#     "precision":          "LLM-judge labels → scikit-learn precision_score",
#     "recall":             "LLM-judge labels → scikit-learn recall_score",
#     "accuracy":           "LLM-judge (Groq llama-3.3-70b) correctness rate",
#     "class_balance":      "value_counts min/max ratio",
#     "avg_confidence":     "mean of confidence/probability column",
#     "avg_latency_ms":     "mean of latency/response_time column",
#     "step_success_rate":  "keyword inference from output text",
#     "task_completion_rate": "keyword inference from output text",
#     "error_rate":         "keyword inference from output text",
#     "avg_step_latency_ms": "mean of latency column",
...
#             # Inject judge-derived accuracy into computed_values
