...
# ):
#     """
#     Upload one or more knowledge base files (PDF / TXT / MD / DOCX / CSV).

#     The form field name MUST be 'files' (supports multiple).
#     The extracted text is chunked (~500 words) and stored in MongoDB.
#     """
#     from app.services.sdcc.models.summarization import SummarizationEvaluator
#     import io as _io

#     # Guard: FastAPI may pass a single UploadFile if only one file is sent;
#     # normalise to a list so the loop always works.
...
#     # Summarization / RAG text-pair metrics
...
#         # Inject judge labels back into df so metric calculators can use them
#         if judge_labels:
...
#     # ── Compute NLP/ML metrics from raw text ───────────────────────────────────
#     # Pass kb_chunks so ROUGE/BLEU/BERTScore can use them as references
#     # and faithfulness/hallucination can use them as context
...
#             # Inject judge-derived accuracy (always authoritative)
...
def ingest_logs(
    ai_name: str,
    file: UploadFile = File(...),
    merge: bool = False,   # if True, append new rows to existing records
    current_user=Depends(get_current_user),
):
    """
    Ingest a CSV log file for the given AI system.

    merge=False (default): replace existing records (original behaviour).
    merge=True:            append new rows to any existing sample_records so
                           that uploaded logs and blackbox probe CSVs can both
                           contribute to the same evaluation run.
    """
    result = run_sdcc_pipeline(ai_name, file, current_user)

    if merge:
        # Fetch existing records and combine with the new ones
...
def evaluate_ai(
    ai_name: str,
    include_blackbox: bool = True,   # auto-merge latest blackbox probe results
    current_user=Depends(get_current_user),
):
    """
    Evaluate the AI system.

    include_blackbox=True (default): the most recent blackbox audit's probe
    results are automatically merged with the uploaded log records so both
    data sources contribute to the evaluation in a single run.
    Set include_blackbox=False to evaluate only the uploaded logs.
    """
    sdcc_doc = sdcc_collection.find_one({
        "ai_name":  ai_name,
        "owner_id": str(current_user["_id"]),
    ...
    # ── FIX: Auto-merge blackbox probe results ─────────────────────────────────
    # Pull the most recent blackbox audit for this AI system and convert its
    # probe results into the same input/output format as uploaded logs.
    # This means running blackbox + uploading logs both count toward the audit.
        ...
        if bb_doc and bb_doc.get("probe_results"):
            for pr in bb_doc["probe_results"]:
                ...
                if prompt and response and not response.startswith("["):
                    # Exclude error placeholders like "[HTTP 401]" or "[ERROR]"
