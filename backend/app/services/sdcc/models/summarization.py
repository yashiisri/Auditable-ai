...
========================================
Evaluator for text summarization models.
...
-----------------------
  ...
  - bertscore         BERTScore F1 (semantic similarity)
  - faithfulness      Factual consistency with source document
  - reference_coverage Proportion of records with reference summaries

TAF emphasis
------------
  Reliability is the highest-weighted principle (ROUGE/BLEU are reliability
  metrics). Explainability is second because faithfulness tells stakeholders
...
class SummarizationEvaluator(BaseEvaluator):
    ...
    # Thresholds based on CNN/DailyMail and XSum benchmark ranges
    THRESHOLDS = {
    ...
    def model_metrics(self, df: pd.DataFrame) -> dict:
        return {
            "rouge_l": self._metric_result(
                self._mean(df, "rouge_l", "rouge_lsum", "rougel", "rouge_l_f1"),
                "ROUGE-L F1 — measures longest common subsequence overlap with reference summary",
                "rouge_l",
            ),
            "rouge_1": self._metric_result(
                self._mean(df, "rouge_1", "rouge1", "rouge_1_f1"),
                "ROUGE-1 unigram overlap with reference summary",
                "rouge_1",
            ),
            "rouge_2": self._metric_result(
                self._mean(df, "rouge_2", "rouge2", "rouge_2_f1"),
                "ROUGE-2 bigram overlap with reference summary",
                "rouge_2",
            ),
            "bleu": self._metric_result(
                self._mean(df, "bleu", "bleu_score", "bleu4"),
                "BLEU n-gram precision score against reference summaries",
                "bleu",
            ),
            "bertscore": self._metric_result(
                self._mean(df, "bertscore", "bert_score", "bert_f1",
                           "bertscore_f1", "bs_f1"),
                "BERTScore F1 — semantic similarity to reference summary using contextual embeddings",
                "bertscore",
            ),
            "faithfulness": self._metric_result(
                self._mean(df, "faithfulness", "factual_consistency",
                           "groundedness", "faithfulness_score"),
                "Factual consistency of the summary with the source document",
                "faithfulness",
            ),
            "reference_coverage": self._metric_result(
                self._coverage(df, "reference_summary", "reference", "ground_truth",
                                "target", "ref_summary"),
    ...
    def taf_principles(self, diagnostics: dict, logs_count: int, metrics: dict) -> dict:
        ...
        # 2. Explainability — faithfulness is the primary explainability signal
        e = {
        ...
        # 3. Fairness — coverage of source documents matters for representational fairness
        f = {
        ...
        # 5. Data Integrity — reference summary availability is crucial
        di = {
        ...
        # 6. Reliability — ROUGE scores are the primary reliability signals
        rouge_score = c(best_rouge_val * 200) if has_rouge else 30   # ROUGE-L ~0.40 → 80
        r = {
        ...
        # 8. Privacy — source documents often contain sensitive content
        pr = {
        ...
        su = {
            ...
            "Compute Proxy Score":   50,   # summarization models are moderately compute-intensive
            "Redundancy Elimination": s["dup_penalty"],
            "Resource Optimisation": c(s["schema_score"] * 0.6 + 40),
        }

        # 10. Safety — for summarization: misleading summaries in medical/legal/financial domains cause harm
        sf = {
