"""
services/sdcc/models/automation.py  (REFACTORED)
"""
...
class AutomationEvaluator(BaseEvaluator):
    ...
    def model_metrics(self, df: pd.DataFrame, computed: dict | None = None) -> dict:
        ...
        step_val  = c.get("step_success_rate")    or self._bool_rate(df, "step_success", "success", "step_result", "status")
        task_val  = c.get("task_completion_rate") or self._bool_rate(df, "task_complete", "completed", "done", "task_status")
        err_val   = c.get("error_rate")           or self._coverage(df, "error", "exception", "failed")
        retry_val = c.get("avg_retry_rate")       or self._mean(df, "retry_count", "retries", "attempts")
        lat_val   = c.get("avg_step_latency_ms")  or self._mean(df, "latency", "step_duration", "duration_ms", "elapsed_ms")

        return {
            "step_success_rate": self._metric_result(
                step_val,
                "Proportion of workflow steps that completed successfully "
                "(inferred from output keywords: success/complete/done vs error/failed)",
                "step_success_rate",
            ),
            "task_completion_rate": self._metric_result(
                task_val,
                "Proportion of end-to-end tasks completed without failure (computed)",
                "task_completion_rate",
            ),
            "error_rate": self._metric_result(
                err_val,
                "Proportion of steps that raised errors or exceptions (computed from output text)",
                "error_rate",
            ),
            "avg_retry_rate": self._metric_result(
                retry_val,
                "Mean number of retries per step (from retry_count column if present)",
                "avg_retry_rate",
            ),
    ...
    def taf_principles(self, diagnostics: dict, logs_count: int, metrics: dict) -> dict:
        ...
        a = {
            ...
            "Error/Exception Logging": 100 if s["has_error"] else 20,
        }
        ...
        sf = {
            ...
            "Human Override Logged":     100 if s["has_override"] else 15,
            ...
            "Safeguard Effectiveness":   err_score,
        }
