...
//   const userContext      = raw?.operator_change_context || raw?.user_context || null;
...
//   const [showPhase1Delta, setShowPhase1Delta] = useState(false);
...
//   const [chainTooltip, setChainTooltip] = useState<{ node: any; x: number; y: number } | null>(null);
...
//         {/* ── DELTA BANNER — only for re-run audits ─────────────────────────── */}
//         {isRerun && deltaSummary && (
//           <div className="card" style={{ marginBottom: 24, overflow: "hidden", ...fade(0.08) }}>
//             {/* Banner top strip */}
//             <div style={{ background: "linear-gradient(135deg, #1E3A8A, #2563EB)", padding: "16px 28px", display: "flex", alignItems: "center", gap: 12 }}>
//               <span style={{ fontSize: 18 }}>↺</span>
//               <div>
//                 <div style={{ fontSize: 14, fontWeight: 800, color: "white" }}>Re-run Comparison</div>
//                 <div style={{ fontSize: 12, color: "rgba(255,255,255,0.7)", marginTop: 2 }}>
//                   {userContext ? `"${userContext.slice(0, 100)}${userContext.length > 100 ? "…" : ""}"` : "Compared against prior audit baseline"}
//                 </div>
//               </div>
//             </div>
...
//                 </div>
//                 <div style={{ fontSize: 10, fontWeight: 700, color: "#64748B", marginTop: 5, textTransform: "uppercase", letterSpacing: "0.05em" }}>Score Change</div>
//               </div>
...
//             {/* Per-principle delta table */}
//             {principleDeltas.length > 0 && (
//               <div style={{ padding: "0 28px 20px" }}>
//                 <div style={{ fontSize: 11, fontWeight: 700, color: "#2563EB", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 10 }}>
//                   Principle Movement
//                 </div>
//                 <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
//                   {principleDeltas.filter(pd => !pd.not_reprobed).map((pd) => {
//                     const mlColors: Record<string, { color: string; bg: string; label: string }> = {
//                       RESOLVED:  { color: "#059669", bg: "#DCFCE7", label: "RESOLVED ✓" },
//                       REGRESSED: { color: "#DC2626", bg: "#FEE2E2", label: "REGRESSED ↓" },
...
//             {/* Phase 1 fingerprint delta collapsible */}
//             {phase1Delta && (
//               <div style={{ padding: "0 28px 20px" }}>
//                 <button
...
//                           <th style={{ padding: "8px 12px", textAlign: "left", fontWeight: 700, color: "#64748B" }}>Dimension</th>
//                           <th style={{ padding: "8px 12px", textAlign: "left", fontWeight: 700, color: "#64748B" }}>Prior Status</th>
//                           <th style={{ padding: "8px 12px", textAlign: "left", fontWeight: 700, color: "#64748B" }}>Current Status</th>
//                         </tr>
//                       </thead>
//                       <tbody>
//                         {phase1Delta.per_dimension_changes.map((ch: any, i: number) => (
...
//                   <span>Conflicts: <strong style={{ color: phase1Delta.conflict_count_change > 0 ? "#DC2626" : "#059669" }}>{phase1Delta.conflict_count_prior} → {phase1Delta.conflict_count_current} ({phase1Delta.conflict_count_change >= 0 ? "+" : ""}{phase1Delta.conflict_count_change})</strong></span>
//                   <span>Domain: <strong>{phase1Delta.domain_prior || "—"} → {phase1Delta.domain_current || "—"}</strong>{phase1Delta.domain_changed ? " ⚠" : " ✓"}</span>
//                 </div>
//               </div>
//             )}
...
//             {/* Audit History Timeline — score progression across all runs */}
//             {auditChain.length > 1 && (
//               <div style={{ padding: "0 28px 24px" }} onClick={() => setChainTooltip(null)}>
//                 <div style={{ fontSize: 11, fontWeight: 700, color: "#2563EB", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 12 }}>
//                   Audit History — {auditChain.length} runs
//                 </div>
//                 <div style={{ overflowX: "auto", paddingBottom: 4 }}>
//                   <div style={{ display: "flex", alignItems: "center", minWidth: auditChain.length * 130 }}>
//                     {auditChain.map((node: any, i: number) => {
//                       const sc2 = (s: number) => s >= 75 ? "#059669" : s >= 50 ? "#2563EB" : "#DC2626";
//                       const seq = node.rerun_sequence ?? 1;
//                       const isBaseline = !node.parent_audit_id;
//                       const scopeColors: Record<string, string> = { full: "#7C3AED", targeted: "#2563EB", phase1_only: "#059669" };
//                       const scopeColor = node.rerun_scope ? (scopeColors[node.rerun_scope] || "#94A3B8") : "#94A3B8";
//                       const change = node.delta_summary?.overall_score_change;
//                       const isActive = node.audit_id === raw?.audit_id;
//                       return (
//                         <div key={node.audit_id} style={{ display: "flex", alignItems: "center" }}>
//                           {/* Node */}
//                           <div
...
//                               setChainTooltip(prev => prev?.node.audit_id === node.audit_id ? null : { node, x: rect.left, y: rect.bottom + 8 });
//                             }}
//                           >
//                             {/* Score ring */}
//                             <div style={{ position: "relative" }}>
//                               <svg width={52} height={52} viewBox="0 0 52 52">
//                                 <circle cx={26} cy={26} r={19} fill="none" stroke={isActive ? "#BFDBFE" : "#E2E8F0"} strokeWidth={5}/>
//                                 <circle cx={26} cy={26} r={19} fill="none" stroke={sc2(node.overall_score)} strokeWidth={5}
//                                   strokeDasharray={`${(node.overall_score / 100) * 119.4} 119.4`} strokeLinecap="round"
//                                   transform="rotate(-90 26 26)"/>
//                                 <text x={26} y={30} textAnchor="middle" fontSize={11} fontWeight="900" fill={sc2(node.overall_score)}>{node.overall_score}</text>
//                               </svg>
//                               <div style={{ position: "absolute", top: -3, right: -3, width: 16, height: 16, borderRadius: "50%", background: isBaseline ? "#2563EB" : "#7C3AED", border: "2px solid white", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, fontWeight: 800, color: "white" }}>
//                                 {seq}
//                               </div>
//                               {isActive && (
//                                 <div style={{ position: "absolute", bottom: -2, left: "50%", transform: "translateX(-50%)", width: 6, height: 6, borderRadius: "50%", background: "#2563EB" }} />
//                               )}
//                             </div>
//                             {/* Label */}
//                             <div style={{ textAlign: "center" }}>
//                               <div style={{ fontSize: 9.5, fontWeight: 700, color: isBaseline ? "#2563EB" : "#7C3AED" }}>{isBaseline ? "Baseline" : `Run #${seq}`}</div>
//                               {node.rerun_scope && <div style={{ fontSize: 8.5, fontWeight: 600, color: scopeColor }}>{node.rerun_scope}</div>}
//                               <div style={{ fontSize: 8.5, color: "#94A3B8" }}>{new Date(node.created_at).toLocaleDateString("en-GB", { day: "2-digit", month: "short" })}</div>
//                             </div>
//                           </div>
//                           {/* Connector */}
...
//                                 <div style={{ height: 1, width: 34, background: "#E2E8F0" }}/>
//                                 <div style={{ width: 0, height: 0, borderTop: "3px solid transparent", borderBottom: "3px solid transparent", borderLeft: "5px solid #CBD5E1" }}/>
//                               </div>
//                             </div>
//                           )}
//                         </div>
...
//                         {(chainTooltip.node.rerun_sequence ?? 1) === 1 ? "Baseline" : `Re-run #${chainTooltip.node.rerun_sequence}`}
//                       </span>
//                       <button onClick={() => setChainTooltip(null)} style={{ background: "none", border: "none", color: "#94A3B8", cursor: "pointer", fontSize: 15, lineHeight: 1, padding: 0 }}>×</button>
//                     </div>
//                     <div style={{ fontSize: 11, color: "#64748B", lineHeight: 1.8 }}>
//                       <div>Score: <strong style={{ color: chainTooltip.node.overall_score >= 75 ? "#059669" : chainTooltip.node.overall_score >= 50 ? "#2563EB" : "#DC2626" }}>{chainTooltip.node.overall_score}</strong></div>
//                       <div>Risk: <strong>{chainTooltip.node.risk_level}</strong></div>
//                       {chainTooltip.node.rerun_scope && <div>Scope: <strong>{chainTooltip.node.rerun_scope}</strong></div>}
//                       {chainTooltip.node.probes_run && <div>Probes: <strong>{chainTooltip.node.probes_run}</strong></div>}
//                       {chainTooltip.node.delta_summary && (
...
//                         </div>
//                       )}
//                       {chainTooltip.node.audit_id !== raw?.audit_id && (
//                         <button
//                           onClick={() => {
//                             setChainTooltip(null);
//                             const token = localStorage.getItem("token");
//                             fetch(`http://localhost:8000/blackbox/audit/${chainTooltip.node.audit_id}`, { headers: { Authorization: `Bearer ${token}` } })
...
//                     <div style={{ fontSize: 20, fontWeight: 900, color: r.overall_score >= 75 ? "#059669" : r.overall_score >= 50 ? "#2563EB" : "#DC2626" }}>{r.overall_score}</div>
//                     <div style={{ fontSize: 9.5, color: "#94A3B8" }}>Overall</div>
//                   </div>
//                   <div style={{ textAlign: "center" as const }}>
//                     <div style={{ fontSize: 14, fontWeight: 800, color: r.risk_level === "Low" ? "#059669" : r.risk_level === "Moderate" ? "#D97706" : "#DC2626" }}>{r.risk_level}</div>
//                     <div style={{ fontSize: 9.5, color: "#94A3B8" }}>Risk</div>
//                   </div>
//                   {r.findings?.length > 0 && (
//                     <div style={{ textAlign: "center" as const }}>
//                       <div style={{ fontSize: 14, fontWeight: 800, color: "#DC2626" }}>{r.findings.length}</div>
...
const CC: Record<string, string> = {
  ...
  "Good Alignment":    "#059669",
  "Partial Alignment": KPMG_MID,
  "Limited Alignment": "#64748B",
  // Legacy values kept for backward compat
  Compliant: "#059669", "Certified Ready": "#059669", Aligned: "#059669",
  Conditional: KPMG_MID, Assessed: KPMG_LIGHT, Partial: "#64748B",
};
    ...
    <div style={{ minHeight: "100vh", background: "#F1F5F9", fontFamily: "'Plus Jakarta Sans', sans-serif", color: "#1E293B", paddingBottom: 80 }}>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "32px 24px" }}>
        ...
        {(() => {
          ...
          const medCount  = allFindings.filter(f => f.severity === "Medium").length;
          const worstOverall = allFindings.length ? worstSev(allFindings.map(f => ({ severity: f.severity }))) : "Pass";
          const wsc = worstOverall === "High" ? "#64748B" : worstOverall === "Medium" ? KPMG_MID : worstOverall === "Low" ? "#059669" : "#64748B";
