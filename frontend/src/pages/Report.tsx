...
//         <p style={{ color:"#64748B", fontSize:18 }}>No report data found.</p>
//         <button style={{ padding:"14px 32px", background:"white", border:"1px solid #E2E8F0", color:"#374151", borderRadius:12, cursor:"pointer", fontSize:15, fontWeight:600 }} onClick={() => navigate("/dashboard")}>Back to Dashboard</button>
//       </div>
//     );
//   }
...
//         @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800;900&display=swap');
//         * { box-sizing:border-box; margin:0; padding:0; }
//         .rpt-card { background:white; border-radius:16px; border:1px solid #E3EAF3; box-shadow:0 2px 12px rgba(0,51,141,0.05); }
...
//         .section-divider { display:flex; align-items:center; gap:16px; margin:12px 0 4px; }
//         .section-divider::before,.section-divider::after { content:''; flex:1; height:1px; background:linear-gradient(90deg,#E3EAF3,transparent); }
//         .section-divider::after { background:linear-gradient(90deg,transparent,#E3EAF3); }
//       `}</style>
...
//           <button onClick={() => navigate("/dashboard")} style={{ display:"flex", alignItems:"center", gap:7, background:"none", border:"none", cursor:"pointer", color:"#6B7C93", fontSize:13, fontWeight:600, padding:"0 4px", transition:"color 0.15s" }}
//             onMouseEnter={e=>(e.currentTarget as HTMLElement).style.color=KPMG_BLUE}
//             onMouseLeave={e=>(e.currentTarget as HTMLElement).style.color="#6B7C93"}>
//             <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
//             Dashboard
//           </button>
//           <div style={{ width:1, height:20, background:"#E3EAF3" }} />
//           <span style={{ fontSize:15, fontWeight:900, letterSpacing:"-0.03em", background:`linear-gradient(135deg,${KPMG_BLUE},${KPMG_MID})`, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>Auditable AI</span>
//           <span style={{ fontSize:10, fontWeight:700, padding:"2px 9px", background:"#E6F2FB", color:KPMG_MID, borderRadius:20, letterSpacing:"0.06em", textTransform:"uppercase" }}>Governance Report</span>
//         </div>
//         <div style={{ display:"flex", gap:10, alignItems:"center" }}>
//           <button onClick={handleDownloadPDF} disabled={pdfLoading}
//             style={{ padding:"8px 20px", background:pdfLoading?"#F1F5F9":`linear-gradient(135deg,${KPMG_BLUE},${KPMG_MID})`, border:"none", borderRadius:8, color:pdfLoading?KPMG_MID:"white", cursor:pdfLoading?"not-allowed":"pointer", fontSize:13, fontWeight:700, boxShadow:pdfLoading?"none":`0 4px 14px ${KPMG_BLUE}28` }}>
//             {pdfLoading ? "Generating..." : "Download PDF"}
//           </button>
//           <button style={{ padding:"8px 20px", background:"white", border:"1px solid #E3EAF3", color:"#6B7C93", borderRadius:8, cursor:"pointer", fontSize:13, fontWeight:600 }}
//             onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.borderColor=KPMG_MID; (e.currentTarget as HTMLButtonElement).style.color=KPMG_MID; }}
...
//       {/* HERO */}
//       <div style={{ background:`linear-gradient(135deg,${KPMG_BLUE} 0%,${KPMG_MID} 55%,${KPMG_LIGHT} 100%)`, padding:"36px 40px 28px", ...fade(0) }}>
...
//             </div>
//           </div>
//           <div style={{ textAlign:"center", background:"rgba(255,255,255,0.1)", borderRadius:16, padding:"18px 28px", backdropFilter:"blur(8px)", border:"1px solid rgba(255,255,255,0.12)" }}>
//             <div style={{ fontSize:56, fontWeight:900, lineHeight:1, color:"white", letterSpacing:"-0.04em" }}>{r.overall_score}</div>
...
//         <div style={{ display:"flex", flexDirection:"column", gap:24, ...fade(0) }}>

//             {/* Stat row */}
//             <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:14 }}>
//               {[
//                 { label:"Logs Evaluated", val:r.logs_evaluated, color:KPMG_MID, icon:"📋" },
//                 { label:"Data Quality", val:`${r.data_quality_score}%`, color:"#059669", icon:"✓" },
//                 { label:"Structural Risk", val:r.structural_risk, color:r.structural_risk==="Low"?"#059669":r.structural_risk==="Moderate"?KPMG_MID:"#DC2626", icon:"⚡" },
//                 { label:"Principles", val:pkeys.length, color:KPMG_BLUE, icon:"◈" },
//                 { label:"Findings", val:r.findings?.length||0, color:(r.findings?.length||0)>0?"#DC2626":"#059669", icon:"⚑" },
//                 ...(r.llm_judge?.accuracy!=null?[{ label:"Accuracy", val:`${Math.round(r.llm_judge.accuracy*100)}%`, color:Math.round(r.llm_judge.accuracy*100)>=80?"#059669":KPMG_MID, icon:"🎯" }]:[]),
//               ].map((s,i) => (
//                 <div key={i} className="rpt-hover" style={{ background:"white", borderRadius:14, border:"1px solid #E3EAF3", padding:"18px 16px", borderTop:`3px solid ${s.color}`, boxShadow:"0 2px 8px rgba(0,51,141,0.04)" }}>
//                   <div style={{ fontSize:26, fontWeight:900, color:s.color, lineHeight:1, marginBottom:6 }}>{s.val}</div>
//                   <div style={{ fontSize:11, color:"#6B7C93", fontWeight:600, letterSpacing:"0.03em" }}>{s.label}</div>
//                 </div>
//               ))}
//             </div>
//             {r.column_warnings && r.column_warnings.length > 0 && (
//               <div style={{ padding:"14px 20px", borderRadius:12, background:"#EFF6FF", border:`1px solid ${KPMG_LIGHT}40`, display:"flex", flexDirection:"column", gap:6 }}>
//                 {r.column_warnings.map((w,i) => (
...
//                 <div style={{ width:36, height:36, borderRadius:9, background:"#EFF6FF", display:"grid", placeItems:"center", color:KPMG_MID }}><SvgBuilding /></div>
//                 <div>
...
//                   return (
//                     <div key={key} className="rpt-hover" style={{ padding:"20px", borderRadius:14, background:alBg, border:`1.5px solid ${alColor}22`, textAlign:"center" }}>
//                       <div style={{ display:"flex", justifyContent:"center", marginBottom:10, color:alColor }}><FwIcon /></div>
...
//                       <div style={{ display:"inline-flex", alignItems:"center", gap:6, padding:"5px 14px", borderRadius:20, fontSize:12, fontWeight:700, color:alColor, background:"white", border:`1.5px solid ${alColor}35` }}>
...
//                 <div style={{ padding:"18px 22px", borderRadius:12, background:"#F8FAFC", border:"1px solid #E3EAF3", borderLeft:`4px solid ${KPMG_MID}` }}>
...
//               <span style={{ fontSize:11, fontWeight:800, letterSpacing:"0.12em", textTransform:"uppercase", color:"#94A3B8", padding:"4px 14px", background:"#F4F7FB", borderRadius:20, border:"1px solid #E3EAF3", whiteSpace:"nowrap" }}>Trusted AI Principles</span>
//             </div>
...
//                   ].map(item => (
//                     <div key={item.label} className="rpt-hover" style={{ background:"white", borderRadius:14, border:"1px solid #E3EAF3", padding:"20px 22px", borderTop:`3px solid ${item.color}`, boxShadow:"0 2px 8px rgba(0,51,141,0.04)" }}>
...
//                               style={{ display:"flex", alignItems:"center", gap:12, padding:"16px", borderRadius:14, background:"white", border:"1.5px solid #E3EAF3", cursor:"pointer", borderTop:`3px solid ${c}` }}>
//                               <div style={{ width:40, height:40, borderRadius:10, flexShrink:0, background:`${c}10`, border:`1px solid ${c}22`, display:"grid", placeItems:"center", color:c }}>{IC?<IC />:<SvgClip />}</div>
//                               <div style={{ flex:1, minWidth:0 }}>
...
//                                   <div style={{ width:`${sc}%`, height:"100%", background:bandColor(sc), borderRadius:99, transition:"width 0.8s ease" }} />
//                                 </div>
//                               </div>
//                               <div style={{ textAlign:"right", flexShrink:0 }}>
//                                 <div style={{ fontSize:22, fontWeight:900, color:bandColor(sc), lineHeight:1 }}>{sc}</div>
//                                 <div style={{ fontSize:9, fontWeight:700, padding:"2px 6px", borderRadius:6, background:bandBg(sc), color:bandColor(sc), marginTop:4, textTransform:"uppercase", letterSpacing:"0.04em" }}>{band(sc)}</div>
//                               </div>
//                             </div>
//                           );
//                         })}
...
//                   ) : selData ? (
//                     <div>
//                       <button onClick={() => setSel(null)} style={{ marginBottom:20, padding:"8px 16px", background:"#EFF6FF", border:`1px solid ${KPMG_LIGHT}40`, color:KPMG_MID, borderRadius:8, cursor:"pointer", fontSize:13, fontWeight:700 }}>All Principles</button>
//                       <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:20, padding:"20px 22px", borderRadius:14, background:`linear-gradient(135deg,${(COLORS[sel]||KPMG_MID)}08,${(COLORS[sel]||KPMG_MID)}03)`, border:`1.5px solid ${(COLORS[sel]||KPMG_MID)}22` }}>
//                         <div style={{ width:50, height:50, borderRadius:13, flexShrink:0, display:"grid", placeItems:"center", background:`${COLORS[sel]||KPMG_MID}12`, border:`1px solid ${(COLORS[sel]||KPMG_MID)}22` }}>{(()=>{ const IC=ICONS[sel]; return IC?<IC />:<SvgClip />; })()}</div>
//                         <div style={{ flex:1 }}>
...
//                           <div style={{ fontSize:10, fontWeight:700, padding:"3px 10px", borderRadius:8, background:bandBg(selData.score), color:bandColor(selData.score), marginTop:6, textTransform:"uppercase", letterSpacing:"0.04em" }}>{band(selData.score)}</div>
//                         </div>
//                       </div>
...
//                         ].map(s => (
//                           <div key={s.label} style={{ padding:"14px 16px", borderRadius:12, background:s.bg, border:`1px solid ${s.color}18` }}>
//                             <div style={{ fontSize:10, color:s.color, textTransform:"uppercase", letterSpacing:"0.07em", fontWeight:700, marginBottom:6 }}>{s.label}</div>
...
//                       <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20, alignItems:"start" }}>
//                         <div>
//                           <div style={{ fontSize:11, fontWeight:700, color:KPMG_MID, textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:12, padding:"8px 12px", background:"#EFF6FF", borderRadius:8 }}>Sub-parameters — hover to inspect</div>
//                           <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
//                             {Object.entries(selData.parameters).map(([param,val]) => {
...
//                               return (
//                                 <div key={param} className="param-row"
//                                   style={{ padding:"14px 16px", borderRadius:12, background:isActive?`${c}05`:"#F8FAFC", border:isActive?`2px solid ${c}35`:"1.5px solid #E3EAF3", cursor:"pointer" }}
...
//                                     <div style={{ textAlign:"right", flexShrink:0 }}>
//                                       <div style={{ fontSize:20, fontWeight:900, color:sc2 }}>{v}</div>
//                                       <div style={{ fontSize:9, fontWeight:700, padding:"1px 6px", borderRadius:6, background:bandBg(v), color:sc2, textTransform:"uppercase", letterSpacing:"0.04em" }}>{band(v)}</div>
//                                     </div>
//                                   </div>
//                                   <div style={{ height:4, background:"#E3EAF3", borderRadius:99 }}>
//                                     <div style={{ width:`${v}%`, height:"100%", borderRadius:99, background:`linear-gradient(90deg,${c},${sc2})`, transition:"width 0.5s ease" }} />
//                                   </div>
//                                 </div>
//                               );
...
//                           </div>
//                         </div>
//                         <div style={{ position:"sticky", top:80, padding:"24px", borderRadius:16, background:"white", border:`2px solid ${(COLORS[sel]||KPMG_MID)}15`, minHeight:280, boxShadow:"0 4px 24px rgba(0,51,141,0.06)" }}>
//                           {activeParam && activeInsight ? (
//                             <>
...
//                               </div>
//                               {(()=>{ const meta=SUB_PARAM_META[activeParam]; const formulaText=meta?.formula||activeInsight.formula; const whyText=meta?.why; if(!formulaText || !isAdmin()) return null; return (
//                                 <div style={{ marginBottom:12, borderRadius:12, overflow:"hidden", border:"1px solid #E3EAF3" }}>
//                                   <div style={{ padding:"8px 14px", background:"#EFF6FF", display:"flex", alignItems:"center", gap:8 }}>
//                                     <span style={{ fontSize:10, padding:"2px 7px", borderRadius:10, background:"#EDE9FE", color:"#7C3AED", fontWeight:700, letterSpacing:"0.04em" }}>ADMIN</span>
...
//                                     <div style={{ padding:"8px 12px", borderRadius:8, background:"#F8FAFC", border:"1px solid #E3EAF3", fontSize:12, color:"#1E293B", lineHeight:1.7 }}>{formulaText}</div>
...
//             {/* ── FINDINGS ── */}
//             <div className="section-divider">
//               <span style={{ fontSize:11, fontWeight:800, letterSpacing:"0.12em", textTransform:"uppercase", color:"#94A3B8", padding:"4px 14px", background:"#F4F7FB", borderRadius:20, border:"1px solid #E3EAF3", whiteSpace:"nowrap" }}>Audit Findings</span>
//             </div>
...
//               {!r.findings?.length ? (
//                 <div style={{ padding:"20px 24px", background:"#F0FDF4", border:"1px solid #86EFAC", borderRadius:12, color:"#166534", fontWeight:600, display:"flex", alignItems:"center", gap:10 }}>
...
//                     return (
//                       <div key={i} style={{ borderRadius:14, background:"white", border:`1.5px solid ${sc}18`, overflow:"hidden" }}>
...
//                             {f.type && <span style={{ fontSize:11, color:"#94A3B8", background:"white", padding:"2px 8px", borderRadius:10, border:"1px solid #E3EAF3" }}>{f.type}</span>}
//                           </div>
//                           <span style={{ color:sc, fontWeight:700, background:"white", padding:"4px 14px", borderRadius:20, fontSize:12, border:`1px solid ${sc}28` }}>{f.severity}</span>
//                         </div>
//                         <div style={{ padding:"16px 20px" }}>
//                           <div style={{ fontSize:11, fontWeight:700, color:"#94A3B8", textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:6 }}>Issue Identified</div>
//                           <p style={{ margin:"0 0 14px", color:"#1E293B", lineHeight:1.7, fontSize:14 }}>{f.issue}</p>
//                           <div style={{ padding:"12px 16px", borderRadius:10, background:"#EFF6FF", border:`1px solid ${KPMG_LIGHT}28` }}>
//                             <div style={{ fontSize:11, fontWeight:700, color:KPMG_MID, textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:5 }}>Recommended Action</div>
//                             <p style={{ margin:0, color:KPMG_BLUE, lineHeight:1.65, fontSize:13 }}>{f.recommendation}</p>
//                           </div>
//                         </div>
...
//             {/* ── DATA & METRICS divider ── */}
//             <div className="section-divider">
//               <span style={{ fontSize:11, fontWeight:800, letterSpacing:"0.12em", textTransform:"uppercase", color:"#94A3B8", padding:"4px 14px", background:"#F4F7FB", borderRadius:20, border:"1px solid #E3EAF3", whiteSpace:"nowrap" }}>Data &amp; Metrics</span>
//             </div>
//             <DataStructuralIntegritySection report={r} />
//             {r.model_metrics && Object.values(r.model_metrics).some(m=>m.value!==null) && (
//               <div className="rpt-card" style={{ padding:"28px 32px" }}>
//                 <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20 }}>
//                   <div style={{ width:36, height:36, borderRadius:9, background:"#EFF6FF", display:"grid", placeItems:"center", color:KPMG_MID }}><SvgGear /></div>
//                   <div>
...
//                     const displayKey = key.replace(/_/g," ").replace(/\b\w/g,c=>c.toUpperCase());
//                     return (
//                       <div key={key} className="rpt-hover" style={{ padding:"18px 16px", borderRadius:14, background:mcBg, border:`1px solid ${mc}18`, display:"flex", flexDirection:"column", gap:8 }}>
//                         <div style={{ fontSize:11, color:"#6B7C93", textTransform:"uppercase", letterSpacing:"0.06em", fontWeight:600 }}>{displayKey}</div>
//                         <div style={{ fontSize:26, fontWeight:800, color:mc }}>{displayVal}</div>
//                         <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
//                           <span style={{ fontSize:11, color:mc, background:"white", border:`1px solid ${mc}35`, padding:"2px 8px", borderRadius:20, fontWeight:700 }}>{m.risk_level}</span>
//                           {m.threshold_low!==undefined && <span style={{ fontSize:10, color:"#94A3B8" }}>threshold: {m.threshold_low}{m.unit?` ${m.unit}`:""}</span>}
//                         </div>
...
//                 <div className="rpt-card" style={{ padding:"28px 32px" }}>
//                   <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20 }}>
//                     <div style={{ width:36, height:36, borderRadius:9, background:"#F0FDF4", display:"grid", placeItems:"center", color:"#059669" }}><SvgSearch /></div>
//                     <div>
...
//                       <div key={label} style={{ padding:"12px 20px", borderRadius:12, background:"#F8FAFC", border:`1px solid ${color}18`, textAlign:"center", minWidth:110 }}>
...
//                       const displayKey = key.replace(/_/g," ").replace(/\b\w/g,c=>c.toUpperCase());
//                       return (
//                         <div key={key} className="rpt-hover" style={{ padding:"14px", borderRadius:12, background:ncBg, border:`1px solid ${nc}15`, display:"flex", flexDirection:"column", gap:6 }}>
//                           <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
//                             <span style={{ fontSize:12, fontWeight:700, color:"#0B1F33" }}>{displayKey}</span>
//                             <span style={{ fontSize:10, fontWeight:700, padding:"2px 8px", borderRadius:10, color:nc, background:"white", border:`1px solid ${nc}28` }}>{ok?"Computed":"Unavailable"}</span>
//                           </div>
//                           <div style={{ fontSize:22, fontWeight:900, color:nc }}>{note.value!==null?note.value.toFixed(4):"—"}</div>
//                           <div style={{ fontSize:10, color:"#94A3B8", lineHeight:1.5 }}>{note.library}</div>
//                         </div>
...
//             {/* ── ACCURACY divider ── */}
//             <div className="section-divider">
//               <span style={{ fontSize:11, fontWeight:800, letterSpacing:"0.12em", textTransform:"uppercase", color:"#94A3B8", padding:"4px 14px", background:"#F4F7FB", borderRadius:20, border:"1px solid #E3EAF3", whiteSpace:"nowrap" }}>Accuracy</span>
//             </div>
...
//                 <div className="rpt-card" style={{ padding:"60px 48px", textAlign:"center" }}>
//                   <div style={{ padding:"20px 24px", background:"#F0FDF4", border:"1px solid #86EFAC", borderRadius:12, color:"#166534", fontWeight:600, display:"flex", alignItems:"center", gap:10, justifyContent:"center" }}>
...
//                     ))}
//                   </div>
//                   <div style={{ padding:"12px 18px", borderRadius:12, background:"#FFFBEB", border:"1px solid #FDE68A" }}>
...
//                           <div style={{ width:36, height:36, borderRadius:9, background:`${pColor}12`, display:"grid", placeItems:"center", color:pColor, flexShrink:0 }}>{IC?<IC />:<SvgClip />}</div>
//                           <div style={{ flex:1 }}>
...
//                             {passRate!==null && (
//                               <div style={{ textAlign:"center", padding:"6px 14px", borderRadius:10, background:passRate>=75?"#F0FDF4":passRate>=50?"#FFFBEB":"#FEF2F2", border:`1px solid ${passRate>=75?"#86EFAC":passRate>=50?"#FCD34D":"#FECACA"}` }}>
...
//                               </div>
//                             )}
//                             <span style={{ padding:"4px 12px", borderRadius:20, fontSize:11, fontWeight:700, color:wc, background:wBg, border:`1px solid ${wc}28` }}>{worst}</span>
//                           </div>
//                         </div>
//                         <div style={{ padding:"16px 24px", display:"flex", flexDirection:"column", gap:10 }}>
//                           {pFindings.map((f,fi) => (
//                             <div key={`f-${fi}`} style={{ padding:"12px 16px", borderRadius:12, background:"#F8FAFC", border:"1px solid #E3EAF3", borderLeft:`3px solid ${f.severity==="High"?"#DC2626":f.severity==="Medium"?"#D97706":"#059669"}` }}>
...
//                                 <span style={{ fontSize:10, fontWeight:700, padding:"2px 8px", borderRadius:10, color:f.severity==="High"?"#DC2626":f.severity==="Medium"?"#D97706":"#059669", background:f.severity==="High"?"#FEF2F2":f.severity==="Medium"?"#FFFBEB":"#F0FDF4" }}>{f.severity}</span>
//                               </div>
...
//                             <div key={`p-${pi}`} style={{ padding:"12px 16px", borderRadius:12, background:p.passed?"#F0FDF4":"#FEF2F2", border:`1px solid ${p.passed?"#86EFAC":"#FECACA"}` }}>
//                               <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:6 }}>
//                                 <span style={{ fontSize:11, fontWeight:700, color:"#6B7C93", textTransform:"uppercase", letterSpacing:"0.06em" }}>Probe {pi+1}</span>
//                                 <span style={{ fontSize:11, fontWeight:700, padding:"2px 8px", borderRadius:10, color:p.passed?"#059669":"#DC2626", background:p.passed?"#DCFCE7":"#FEE2E2" }}>{p.passed?"Pass":"Fail"}</span>
//                               </div>
...
function LLMAccuracySection({ llmJudge, modelLabel }: { llmJudge: LLMJudge; modelLabel: string }) {
  ...
  const JUDGE_META: Record<string, { arch: string; color: string }> = {
    "Groq/Llama-3.3-70B":        { arch: "Open-weight transformer",        color: "#7C3AED" },
    "OpenRouter/Mistral-Large":   { arch: "Mixture-of-experts",             color: "#0091DA" },
    "Together/Qwen2.5-72B":       { arch: "Multilingual instruction-tuned", color: "#059669" },
  };
