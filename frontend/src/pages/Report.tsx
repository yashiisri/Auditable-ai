...
function getParameterInsight(param: string, report: ReportData): ParameterInsight {
  ...
  const map: Record<string, ParameterInsight> = {
    ...
    // ... (all other entries unchanged - keeping the full map for completeness)
    "Model Version Tracking": {
      detail: "Checks whether model version identifiers are present for traceability.",
      calculation: `Set to ${s.hasVersion ? "100 because a version/model_id column was detected" : "30 because no version-tracking column was detected"}.`,
    },
...
function Radial({ score, label, color, size = 90, showLabel = true }: { 
  score: number; 
  label: string; 
  color: string; 
  size?: number; 
...
export default function Report() {
  ...
  // ENHANCED DRILL-DOWN: sorted + mini radials
  const sortedEntries = selData 
    ? Object.entries(selData.parameters || {}).sort(([,a], [,b]) => (b as number) - (a as number))
    : [];
            ...
            <div style={{
              ...
              {!sel ? (
                ...
                <div style={{ width: "100%" }}>
                  ...
                  {/* Overview Grid (Strongest / Weakest + Radial) */}
                  <div style={{
                    display: "grid",
                    gridTemplateColumns: "140px 1fr",
                    gap: 18,
                  ...
                  {/* === FANCY LIVE DRILL-DOWN INSIGHT (enhanced with emojis + monospace formula) === */}
                  {activeParam && activeInsight && (
                    <div style={{
                      marginBottom: 22,
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 14, marginBottom: 12 }}>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 19, fontWeight: 800, marginBottom: 8 }}>{activeParam}</div>
                        ...
                        <div style={{
                          ...
                          <div style={{ fontSize: 26, fontWeight: 900, color: COLORS[sel] || "#00C896", lineHeight: 1 }}>
                            {selData.parameters[activeParam]}
                          </div>
                          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.42)", marginTop: 4 }}>score</div>
                        </div>
                      </div>

                      {/* Progress ring under insight */}
                      <div style={{
                        height: 8,
                        background: "rgba(255,255,255,0.08)",
                        borderRadius: 99,
                        ...
                        <div style={{
                          width: `${selData.parameters[activeParam]}%`,
                          height: "100%",
                          background: `linear-gradient(90deg, ${COLORS[sel] || "#00C896"}, #ffffff)`,
                          borderRadius: 99,
                          boxShadow: `0 0 18px ${(COLORS[sel] || "#00C896")}88`,
                        }} />
                      </div>

                      {/* Insight + Formula blocks */}
                      <div style={{
                        fontSize: 12.5,
                        lineHeight: 1.65,
                        ...
                        <div style={{ fontSize: 10.5, letterSpacing: "0.07em", textTransform: "uppercase", color: "rgba(255,255,255,0.45)", marginBottom: 7, display: "flex", alignItems: "center", gap: 6 }}>
                          📝 INSIGHT
                        </div>
                        {activeInsight.detail}
                      </div>
                  ...
                  {/* === ENHANCED SUB-PARAMETER GRID WITH MINI RADIAL GAUGES === */}
                  <div style={{ marginTop: 8 }}>
                    <div style={{ 
                      ...
                      <span style={{ fontSize: 11, padding: "2px 9px", background: "rgba(0,200,150,0.15)", borderRadius: 999, color: "#00C896" }}>
                        sorted by strength ↓
                      </span>
                    </div>
                    ...
                    <div style={{
                      ...
                      {sortedEntries.map(([param, val]) => {
                        const v = val as number;
                        const c = COLORS[sel] || "#00C896";
                        const sl = v >= 75 ? "Strong" : v >= 50 ? "Moderate" : "Needs Attention";
                        const sc2 = v >= 75 ? "#00C896" : v >= 50 ? "#ffb020" : "#ff4d4d";
                        const isActive = activeParam === param;
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              borderRadius: 18,
                              background: isActive
                                ? `linear-gradient(180deg, ${c}12, rgba(255,255,255,0.04))`
                                : "linear-gradient(180deg, rgba(255,255,255,0.045), rgba(255,255,255,0.02))",
                              border: isActive ? `1.5px solid ${c}60` : "1px solid rgba(255,255,255,0.07)",
                              transition: "all 0.35s cubic-bezier(0.23, 1, 0.32, 1)",
                              cursor: "pointer",
                              boxShadow: isActive 
                                ? `0 0 28px ${c}35, 0 14px 32px rgba(0,0,0,0.25)` 
                  ...
                  <button
                    style={{
                      ...
                      padding: "13px",
                      background: `linear-gradient(180deg, ${(COLORS[sel] || "#00C896")}18, rgba(255,255,255,0.03))`,
                      border: `1px solid ${COLORS[sel] || "#00C896"}55`,
                      color: COLORS[sel] || "#00C896",
                      borderRadius: 14,
                      cursor: "pointer",
                      fontSize: 13.5,
                      fontWeight: 600,
                      boxShadow: `0 14px 32px ${(COLORS[sel] || "#00C896")}18`,
                      transition: "all 0.2s",
                    }}
                    onClick={() => setSel(null)}
                  >
