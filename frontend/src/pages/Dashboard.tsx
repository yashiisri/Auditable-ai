...
export default function Dashboard() {
  ...
  const [bbResult, setBbResult]     = useState<BlackBoxResult|null>(null);
  const [bbError, setBbError]       = useState("");
  const [bbProgress, setBbProgress] = useState(0); void bbProgress;
  const [bbDone, setBbDone]         = useState(false);
            ...
            <div className="db-step-body">
              ...
              {bbDone && bbResult ? (
                ...
                <div className="db-card active">
                  ...
                  <button className="db-btn" style={{ marginTop:12 }} onClick={handleBlackBox} disabled={bbLoading}>
                    {bbLoading?(bbTab==="chat"?"Parsing & Ingesting…":"Running Audit…"):(bbTab==="chat"?"Audit Chat History":"Run Black Box Audit")}
                  </button>
                  {bbLoading && bbTab!=="chat" && (
                    <div style={{ marginTop:14 }}>
                      <div style={{ padding:"16px 18px", background:"linear-gradient(135deg,#EEF4FF,#E8F0FD)", borderRadius:12, border:"1px solid #C7D9F5" }}>
                        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
                          <div style={{ width:8, height:8, borderRadius:"50%", background:"#005EB8", animation:"probePulse 1s ease-in-out infinite" }}/>
                          <div style={{ fontSize:13, fontWeight:700, color:"#00338D" }}>Probing in progress…</div>
                          <div style={{ marginLeft:"auto", fontSize:11, color:"#7A90AB", fontWeight:600 }}>Running governance probes</div>
                        </div>
                        <div style={{ height:4, background:"#C7D9F5", borderRadius:99, overflow:"hidden", position:"relative" }}>
                          <div style={{ position:"absolute", top:0, left:0, height:"100%", width:"35%", background:"linear-gradient(90deg,#00338D,#005EB8,#0091DA)", borderRadius:99, animation:"probeSlide 1.6s ease-in-out infinite" }}/>
                        </div>
                        <div style={{ display:"flex", gap:5, marginTop:12, justifyContent:"center" }}>
                          {Array.from({ length: 8 }).map((_, i) => (
                            <div key={i} style={{ width:7, height:7, borderRadius:"50%", background:"#005EB8", animation:`probePulse 1.2s ease-in-out ${i * 0.15}s infinite` }}/>
                          ))}
                        </div>
                      </div>
                      <style>{`@keyframes probePulse{0%,100%{opacity:0.3;transform:scale(0.8)}50%{opacity:1;transform:scale(1.2)}}@keyframes probeSlide{0%{transform:translateX(-100%)}100%{transform:translateX(300%)}}`}</style>
                    </div>
                  )}
                </div>
              )}
