          ...
          <div className="db-modal-body">
            ...
            {!isChat && (() => {
              ...
              const TIER_TOTALS: Record<string, number> = {
                dev: 55, standard: 130, thorough: 255,
              };
              const TOTAL_PROBES = TIER_TOTALS[tier] ?? 130;
              const fp  = Math.round(TOTAL_PROBES * 0.12);   // fingerprint ~12%
              const cp  = Math.round(TOTAL_PROBES * 0.72);   // control probes ~72%
              const af  = Math.round(TOTAL_PROBES * 0.90);   // after adaptive
              const phaseCumulativeEnd = [fp, cp, af, TOTAL_PROBES, TOTAL_PROBES];
              const probesShown = isDone ? TOTAL_PROBES
                : phase === 0 ? Math.min(Math.round((progress / 100) * fp), fp)
...
export default function Dashboard() {
  ...
  // Phase durations per tier — dev is fastest, thorough is longest
  const TIER_PHASE_DURATIONS: Record<string, number[]> = {
    dev:      [15000, 60000,  10000, 15000, 5000],
    standard: [20000, 120000, 25000, 20000, 5000],
    thorough: [30000, 240000, 40000, 30000, 5000],
  };
  const PHASE_DURATIONS_MS = TIER_PHASE_DURATIONS[bbTier] ?? TIER_PHASE_DURATIONS.standard;
  useEffect(() => {
  ...
  const handleBlackBox = async () => {
    ...
    // Real-time progress polling — queries probe_logs COUNT(*) every 2s
    const TIER_POLL_TOTALS: Record<string, number> = { dev: 55, standard: 130, thorough: 255 };
    let _pollExpected = TIER_POLL_TOTALS[bbTier] ?? 130; // updated when first poll returns
    const iv = setInterval(async () => {
      if (!aiName) return;
      try {
        const prog = await import("../services/api").then(m => m.api.get(`/blackbox/progress/${encodeURIComponent(aiName)}`));
        const { probes_logged, probes_expected, progress_pct } = prog.data;
        if (probes_expected) _pollExpected = probes_expected;
        if (progress_pct !== undefined) {
          setBbProgress(Math.min(progress_pct, 92)); // cap at 92 until done
        }
      } catch {
        // Polling failure is non-fatal — fall back to gentle increment
        setBbProgress(p => (p >= 92 ? p : p + Math.random() * 0.5));
      }
    }, 2000);
                ...
                <div className="db-card active">
                  ...
                  {bbTab === "api" && (
                    <div style={{ marginTop: 14 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: "#7A90A8", letterSpacing: "1px", textTransform: "uppercase", marginBottom: 8 }}>
                        Audit depth
                      </div>
                      <div style={{ display: "flex", gap: 8 }}>
                        {(["dev", "standard", "thorough"] as const).map(t => {
                          const labels: Record<string, { label: string; sub: string; color: string }> = {
                            dev:      { label: "Dev",      sub: "~55 probes · fast",        color: "#64748B" },
                            standard: { label: "Standard", sub: "~130 probes · balanced",   color: "#005EB8" },
                            thorough: { label: "Thorough", sub: "~255 probes · cert-grade", color: "#0B3D91" },
                          };
          ...
          <div className="db-step">
            <div className="db-step-body">
              <SectionHeader
                ...
                subtitle="Upload fine-tuning / training data to unlock 15 additional TAF taxonomy controls — bias in training data, memorisation, membership inference, compute footprint."
                done={trainingDone}
                optional="Optional — unlocks TAF Taxonomy controls"
              />
              {!trainingDone ? (
                <div className="db-card active">
                  <div className="db-card-h">Upload Training / Fine-tuning Data</div>
                  <div className="db-card-d">
                    Upload your model's training or fine-tuning dataset (CSV, XLSX, or JSON). TrustShield analyses it for bias representation, deduplication quality, PII exposure, and compute-size proxies — unlocking 15 controls in the TAF Taxonomy tab that otherwise cannot be evaluated from external probing alone.
                  </div>
                ...
                <div className="db-card" style={{ background: "#F5F3FF", border: "1px solid #DDD6FE" }}>
                  ...
                  <div style={{ fontSize: 11, color: "#7C3AED", marginTop: 10 }}>
                    15 TAF taxonomy controls now unlocked — view them in the TAF Taxonomy tab.
                  </div>
                ...
                {evalLoading && (
                  <div className="db-eval-loader">
                    ...
                    <div className="db-eval-steps">
                      ...
                      <span className="db-eval-step">TAF Taxonomy Mapping</span>
