...
//   const topFinding  = allFindings.find(f => f.severity === "High") ?? allFindings.find(f => f.severity === "Medium") ?? allFindings[0];
//   const visibleFindings = showAllFindings ? allFindings : (topFinding ? [topFinding] : []);
...
//         <>
//           <div className="db-section-lbl" style={{ marginTop:18 }}>Score per principle</div>
//           <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
//             {principleEntries.map(([principle, score]) => {
//               const s = score as number;
//               const barColor = s >= 75 ? "#276749" : s >= 50 ? "#D97706" : "#C53030";
//               return (
//                 <div key={principle} style={{ display:"flex", alignItems:"center", gap:10 }}>
//                   <div style={{ fontSize:11.5, color:"#5A7090", minWidth:118, flex:"0 0 118px", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{principle}</div>
//                   <div style={{ flex:1, height:6, background:"#E3EAF3", borderRadius:6, overflow:"hidden" }}>
//                     <div style={{ width:`${Math.min(s,100)}%`, height:"100%", background:barColor, borderRadius:6, transition:"width 0.7s cubic-bezier(.16,1,.3,1)" }} />
//                   </div>
//                   <div style={{ fontSize:11.5, fontWeight:700, color:barColor, minWidth:28, textAlign:"right" }}>{s}</div>
//                 </div>
//               );
//             })}
//           </div>
...
//             {showAllFindings ? `All findings (${allFindings.length})` : "Top finding"}
//           </div>

//           {visibleFindings.map((f, idx) => (
//             <div key={idx} className={`db-finding ${f.severity === "High" ? "high" : f.severity === "Medium" ? "med" : ""}`}>
//               <div className="db-finding-top">
//                 <div className="db-finding-cat">{f.category}</div>
//                 <span className={`db-finding-sev ${f.severity === "Pass" ? "low" : f.severity.toLowerCase()}`}>{f.severity}</span>
//               </div>
//               {f.probe && <div className="db-finding-probe">Prompt: {f.probe}</div>}
//               <div className="db-finding-issue">{f.issue}</div>
//               {f.recommendation && f.severity !== "Pass" && (
//                 <div className="db-finding-rec">Fix: {f.recommendation}</div>
//               )}
//             </div>
//           ))}
...
//               style={{ marginTop:10, width:"100%", padding:"8px 14px", fontSize:12, fontWeight:700, color:"#005EB8", background:"#EEF4FF", border:"1px solid #C7D9F5", borderRadius:8, cursor:"pointer", fontFamily:"inherit", textAlign:"center" as const }}>
//               {showAllFindings ? "↑ Show less" : `↓ Show all ${allFindings.length} findings`}
//             </button>
//           )}
//         </>
//       )}
...
//                       <input className="db-inp" type="password" placeholder="sk-… / gsk_… / sk-ant-… or any key" value={bbApiKey} onChange={e=>setBbApiKey(e.target.value)} disabled={bbLoading} autoComplete="new-password" />
//                       {bbApiKey && (
//                         <div style={{ marginTop:5, fontSize:11, color:"#005EB8", fontWeight:600 }}>
//                           {bbApiKey.startsWith("gsk_") ? "🟢 Groq detected — endpoint auto-resolved"
//                             : bbApiKey.startsWith("sk-ant-") ? "🟢 Anthropic detected — endpoint auto-resolved"
//                             : bbApiKey.startsWith("sk-or-") ? "🟢 OpenRouter detected — endpoint auto-resolved"
//                             : bbApiKey.startsWith("sk-") ? "🟢 OpenAI-compatible key detected"
//                             : "🔵 Custom key — provide endpoint below if needed"}
//                         </div>
//                       )}
//                     </div>
//                     <div className="db-field">
...
//                       <input className="db-inp" type="url" placeholder="Leave blank for Groq/OpenAI/Anthropic/Mistral — or paste your custom endpoint" value={bbEndpoint} onChange={e=>setBbEndpoint(e.target.value)} disabled={bbLoading} autoComplete="off" />
//                     </div>
//                     <div className="db-info" style={{ fontSize:11 }}>
//                       Supports: OpenAI · Groq · Anthropic · Mistral · OpenRouter · Together AI · Cohere · any OpenAI-compatible API. Enterprise APIs with custom endpoints are fully supported.
//                     </div>
//                   </>}
